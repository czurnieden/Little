/*
  ...
  var Little_construction = function(){
    this.double_int = new DataView(new ArrayBuffer(8));
    this.lgamma_sign = 1;
  };

  var Little = new  Little_construction();

  Such that every Web Worker has its own copy. Makes not much sense, hu?
...
function ksin(x,y,iy){
  ...
  Little.double_int.setFloat64(0, x);
  ix = Little.double_int.getInt32(0);  /* get high word */
  ix &= 0x7fffffff;
  if(ix<0x3e400000){                    /* |x| < 2**-27 */
    if( (x|0) == 0) {return x;} /* generate inexact */
  }
  z       =  x*x;
  v       =  z*x;
...
function kcos(x,y){
  ...
  if(ix<0x3e400000) {                  /* if x < 2**27 */
    if( (x|0) == 0){ return one; }        /* generate inexact */
  }
  z  = x*x;
  ...
  if(ix < 0x3FD33333){                     /* if |x| < 0.3 */
    ...
    if(ix > 0x3fe90000) {               /* x > 0.78125 */
      qx = 0.28125;
              ...
              /* x/4 */
      Little.double_int.setFloat64(0, qx);
      Little.double_int.setUint32(0,ix-0x00200000);
      Little.double_int.setUint32(4,0x0);
      qx = Little.double_int.getFloat64(0);
    }
    hz = 0.5*z-qx;
    a  = one-qx;
    return a - (hz - (z*r-x*y));
...
function sin_pi(x){
  ...
  ix = Little.double_int.getInt32(0); /* get high word */
  ix &= 0x7fffffff;
  if(ix<0x3fd00000) {return ksin(Math.PI*x,zero,0);}
   ...
   */
        ...
        if(z!=y) {                             /* inexact anyway */
            ...
            if(ix>=0x43400000) {
                ...
                n = Little.double_int.getInt32(4);
                n &= 1;
                y  = n;
                n<<= 2;
            }
        ...
        switch (n) {
            case 0:   y =  ksin(Math.PI*y,zero,0); break;
            case 1:
            case 2:   y =  kcos(Math.PI*(0.5-y),zero); break;
            case 3:
            case 4:   y =  ksin(Math.PI*(one-y),zero,0); break;
            case 5:
            case 6:   y = -kcos(Math.PI*(y-1.5),zero); break;
            default:  y =  ksin(Math.PI*(y-2.0),zero,0); break;
            }
        return -y;
}

function lgamma_ieee_754(x){
  var half=  5.00000000000000000000e-01; /* 0x3FE00000, 0x00000000 */
  var one =  1.00000000000000000000e+00; /* 0x3FF00000, 0x00000000 */
  ...
  var t,y,z,nadj=0,p,p1,p2,p3,q,r,w;
  var i = 0|0,hx = 0|0,lx = 0|0,ix = 0|0;


  Little.double_int.setFloat64(0, x);

  hx = Little.double_int.getInt32(0); /* high word */
  lx = Little.double_int.getInt32(4); /* low word */

    /* purge off +-inf, NaN, +-0, and negative arguments */
        Little.lgamma_sign = 1;
        ix = hx&0x7fffffff;
        ...
        if(hx<0) {
            ...
            if(t<zero){ Little.lgamma_sign = -1;}
            x = -x;
        }
    /* purge off 1 and 2 */
